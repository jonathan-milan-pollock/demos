/*
async Task<string> CreateAsync(string videoTitle, string imagesDirectory, string publishingDirectory)
{
  foreach (var image in imagesArray)
      await _tinyPngDomain.TinifyImageAsync(image).ConfigureAwait(false);
