...
@Injectable()
export class WebsitesDropboxUserProvider {
  constructor(
  ...
  findEmail$(refreshToken: string): Observable<WebsitesDropboxUserTable> {
      ...
      map((response) => {
        ...
        if (accountResponse.status != 200)
          throw new BadRequestException(
            'Unable to find websites user account information from Dropbox'
          );
        const user = new WebsitesDropboxUserTable();
  ...
  createOrUpdateUser$(
    user: WebsitesDropboxUserTable
  ): Observable<WebsitesDropboxUserTable> {
    return from(this.userRepository.findAll()).pipe(
      ...
      mergeMap((foundUser) => {
        if (!foundUser) {
          ...
          const updateUser = new WebsitesDropboxUserTable();
