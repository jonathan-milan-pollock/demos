...
const StoriesTemplate = ({ fontmatter, markdownBody }: Props) => {
  return (
...
export const getStaticPaths: GetStaticPaths = async () => {
  const stories = glob.sync('story/*.md');

  const photoOfTheWeekSlugs = stories.map((file) =>
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  const content = await import(`../../markdown/story/${slug}.md`);
