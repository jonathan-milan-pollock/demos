...
const execGenerateSharedTypesLib = (isReady) =>
  Promise.resolve(console.log('#### shared-types library'))
    ...
    .then(() =>
      consoleLogOrExec(
        isReady,
        'npx nx g @nrwl/workspace:lib shared-types --unitTestRunner=jest --tags=scope:shared,type:types'
      )
    );
...
const execGenerateSharedServerLibs = (isReady) =>
  Promise.resolve(console.log('#### shared-server libraries'))
    ...
    .then(() =>
      consoleLogOrExec(
        isReady,
        'npx nx g @nrwl/nest:lib shared-server/types -unitTestRunner=jest --tags=scope:shared-server,type:types'
      )
    )
    .then(() =>
      consoleLogOrExec(
...
const execGenerateWebsiteLibs = (isReady) =>
  Promise.resolve(console.log('#### website libraries'))
    ...
    .then(() =>
      consoleLogOrExec(
        isReady,
        'npx nx g @nrwl/angular:lib website/types --unitTestRunner=jest --tags=scope:website,type:types --prefix=drp'
      )
    )
    .then(() =>
      consoleLogOrExec(
